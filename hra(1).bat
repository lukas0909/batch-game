@echo off
chcp 65001 >nul
color B
cls
echo Vitej na začátku vědomostního kvízu. Za každou správnou odpověď se ti budou připočítávat body, a za každou špatnou        odpověď se tvoje body nezmění. Vyber následující krok.

echo 1 = začni hru
echo 2 = exit

choice /C 12
 if errorlevel 2 (
   cls
   exit
) else if errorlevel 1 (
   cls
    Goto :first
   pause
)

:first
echo První otázka
echo Jak se jmenuje hora, která nejvyšší v Evropě?
set /p odpoved=Zadej odpověď: 
if /i "%odpoved%"=="Elbrus" (
    echo správně, dostáváš .....
    pause
    Goto :second
) else (
    echo špatně
    echo bohužel nedostaneš ani jeden bod
    pause
    Goto :second
)

:second
cls
echo Druhá otázka 
echo Který evropský stát nemá žádné řeky?

echo 1 = Portugalsko
echo 2 = San Marino
echo 3 = Vatikán

choice /C 123
if errorlevel 3 (
   cls
   echo správně, dostáváš .....
   pause 
   Goto :third
) else if errorlevel 2 (
   cls
   echo špatně, nedostaneš body
   pause 
   Goto :third
) else (
   cls
   echo špatně, nedostáváš body
   pause 
   Goto :third
)

:third
cls
echo Zatím se ti daří. Pokud odpovíš na třetí otázku správně dostaneš 3 body.

:fourth
cls 
echo Třetí otázka 
echo Jaký je největší orgán lidského těla?
set /p odpoved=Zadej odpověď:
if /i "%odpoved%"=="kůže" (
    echo správně, dostáváš ......
    pause
    Goto :fifth
) else (
    echo špatně, nedostáváš body
    pause
    Goto :fourth
)

:fifth
cls 
echo Výborně, dostal jsi se ke čtvrté otázce. Pokud odpovíš správně, dostaneš 3 body.

:sixth
cls 
echo Čtvrtá otázka
echo Jak se jmenuje největší poušť světa?
 
echo 1 = Sahara
echo 2 = Antarktida
echo 3 = Gobi

choice /C 123
if errorlevel 3 (
   cls 
   echo špatně, nedostáváš body
   pause
   Goto :seventh
) else if errorlevel 2 (
   cls 
   echo správně, dostáváš ..... 
   pause
   Goto :seventh
) else if errorlevel 1 (
   cls 
   echo špatně, nedostáváš body
   pause
   Goto :seventh
)  

:seventh
cls 
echo Blížíš se ke konci kvízu. Zapoj všechny mozkové závity.

:eighth
cls
echo Pátá otázka
echo Který živočich má největší mozek v poměru k tělesné hmotnosti?

echo 1 = Delfín
echo 2 = Člověk
echo 3 = Mravenec

choice /C 123
if errorlevel 3 (
   cls 
   echo správně, dostáváš .....
   pause
   Goto :ninth
) else if errorlevel 2 (
   cls 
   echo špatně, nedostáváš body
   pause
   Goto :ninth
) else if errorlevel 1 (
   cls 
   echo špatně, nedostáváš body
   pause
   Goto :ninth 
)

:ninth
cls
echo Šestá otázka
echo 
